...
#   return(tmp_b)
# }
mymvn <- function (n = 1, mu = rep(0, k), Sigma) 
{
  k <- ncol(Sigma)
...
b_update <- function(X, Y, Z, beta, bsigma, Sigma, XtX, XtY) {
  #tmp_b <- matrix(nrow = ncol(X), ncol = ncol(Y))
  #bmeans <- 0
  bsigs <- vector("list", ncol(X))
  #bsigs <- array(dim = c(ncol(Y), ncol(Y), ncol(X)))
  bmeans <- c(t(Z) * t(solve(XtX + 1/bsigma * diag(ncol(X))) %*% XtY))
  for(colx in 1:ncol(X)) {
    #m_gj <- as.numeric(solve(XtX[colx] + 1/bsigma)) * (t(X[,colx]) %*% (Y - eigenMapMatMult(X[,-colx], beta[-colx,])))
    #m_gj <- Z[colx,] * m_gj
    #bsigs <- c(bsigs, m_gj)
    sig_gj <- (Z[colx,] %*% t(Z[colx,])) * (as.numeric(solve(XtX[colx,colx] + (1/bsigma)))) * Sigma + (1 - Z[colx,] %*% t(Z[colx,])) * (bsigma * Sigma)
    sig_gj[which(Z[colx,]==1),which(Z[colx,]!=1)] <- sqrt((1/bsigma)  * as.numeric(solve(XtX[colx,colx] + 1/bsigma))) * (bsigma * Sigma)[which(Z[colx,]==1),which(Z[colx,]!=1)]
    sig_gj[which(Z[colx,]!=1),which(Z[colx,]==1)] <- sqrt((1/bsigma)  * as.numeric(solve(XtX[colx,colx] + 1/bsigma))) * (bsigma * Sigma)[which(Z[colx,]!=1),which(Z[colx,]==1)]
    bsigs[[colx]] <- sig_gj
    #tmp_b[colx,] <- MASS::mvrnorm(n = 1, mu = m_gj, Sigma = sig_gj)
  }
  return(matrix(mymvn(n = 1, mu = bmeans, Sigma = as.matrix(do.call("bdiag", bsigs))), nrow = ncol(X), ncol = ncol(Y), byrow = TRUE))
}
...
# }
bsigma_update <- function(X, Y, beta, Sigma) {
  return(LaplacesDemon::rinvgamma(n = 1, shape = 0.5 * ncol(X) * ncol(Y), scale = 0.5 * pracma::Trace(beta %*% solve(Sigma) %*% t(beta))))
}
...
#     calc1 <- (-0.5) * pracma::Trace(eigenMapMatMult(eigenMapMatMult(Yng - XB, SSigma), t(Yng - XB)))
...
# }
Sigma_update <- function(X, Y, beta, b, bsigma) {
  return(LaplacesDemon::rinvwishart(nu = ncol(X) + ncol(Y) + nrow(Y), S = diag(ncol(Y)) + eigenMapMatMult(t(Y - eigenMapMatMult(X, beta)), (Y - eigenMapMatMult(X, beta))) + (1/bsigma) * eigenMapMatMult(t(b), b)))
  #return(cIRT::riwishart(df = ncol(X) + ncol(Y) + nrow(Y), S = diag(ncol(Y)) + t(Y - eigenMapMatMult(X, beta)) %*% (Y - eigenMapMatMult(X, beta)) + (1/bsigma) * eigenMapMatMult(t(b), b)))
}
